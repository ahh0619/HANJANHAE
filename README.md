...
<details>
<summary><strong>레그(Leg) 시스템</strong></summary>
<br>
오픈AI 어시스턴스 툴과의 연동 과정에서, 데이터 처리와 분석 로직을 관리·연결하는 미들웨어 역할을 담당하는 시스템입니다.<br>
<br>예컨대 설문조사나 검색 키워드 등 사용자 취향 정보를 입력받으면,<br>
레그 시스템이 이를 적절히 전처리·분류한 뒤 오픈AI 어시스턴스로 전달합니다.<br>

<br>이후 오픈AI 어시스턴스에서 응답받은 결과를 다시 레그 시스템이 취합·가공하여,  <br>
사용자가 쉽게 이해하고 활용할 수 있는 형태로 제공하게 됩니다.

즉, “사용자 입력 → 레그 시스템 → 오픈AI 어시스턴스 → 레그 시스템 → 최종 사용자”의 흐름으로,  <br>
